...
graph.names = c(
  "0"="Underlying graph $\\mathcal{G}$",
  "0.3"="Underlying graph $\\mathcal{H}$"
)
...
p1 <- df %>%
  ...
  labs(y="Rejection rate", x="Number of observations", colour = "Sample size $n$", fill = "Sample size $n$")+
...
graph.names = c(
  "null"="Underlying graph $\\mathcal{G}$",
  "alternative"="Underlying graph $\\mathcal{H}$"
)
df$Hypothesis <- factor(df$Hypothesis, levels=names(graph.names), labels = graph.names)

df$Test <- as.factor(df$Test)
levels(df$Test) <- c(Resampling1 = 'Resampling ($m=\\sqrt{n}$)',
