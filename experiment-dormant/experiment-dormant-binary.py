...
def fit_m(X, p=None, n_iter=10, threshold=0.29):
    ...
    for m in m_size:
        psi_alter = ShiftedTester(wf, fisher_test, rate(1, c=m / n),
                                  replacement=False)
...
def conduct_experiment(i=None):
    ...
    for n in n_range:
        ...
        try:
            ...
            if mode == 'pvalopt':
                # optimize weight
                tester = lambda w: ShiftedTester(w, fisher_test, rate(pow, c=1), replacement=True)
            ...
            psi_null = ShiftedTester(wf_null, fisher_test,
                                     rate_null, replacement=False)
            psi_alter = ShiftedTester(wf_alter, fisher_test,
...
if __name__ == '__main__':
    ...
    df.to_csv("experiment-dormant-binary.csv")
