...
# Load
df <- read_delim("experiment-a2.csv", delim=",", col_types = cols(m="n", n = "f", causal_effect="f")) %>%
  subset(!is.na(pval)) %>%
...
effect_names <- c("0.0" = "$X \\not\\rightarrow Y$", "0.4" = "$X \\rightarrow Y$")
...
###
df_hyp <- read_delim("experiment-a2-ci.csv", delim=",", col_types = cols(idx="f", m="n", n = "f", causal_effect="f"))
df_hyp$causal_effect <- factor(df_hyp$causal_effect, levels=names(effect_names), labels = effect_names)

# Setup tikz
path = "experiment-a2-a3"
...
p1 <- df %>%
  ...
  geom_point(aes(y=meanp), data=max_points, size=4, shape=1, show.legend=F) +
...
p2 <- df %>%
  ...
  scale_x_continuous(trans="log10") +
  geom_point(aes(y=meanhyp), shape=1, data=max_points, size=4) +
  scale_y_continuous(labels = function(z){paste0(100*z, "\\%")}, limits = c(0, 1)) +
...
#####
# Load
df.a3 <- read_delim("experiment-a3.csv", delim=",", col_types=cols(CausalEffect="f", n="f"))
