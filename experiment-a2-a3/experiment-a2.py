...
# Test function
def T(X): return 1*(sm.OLS(X[:, 2], sm.tools.add_constant(X[:, 0])).fit().pvalues[1] < 0.05)


def experiment(i=1):
    psi = ShiftedTester(weight, T, replacement=False, verbose=False)
    res = []
    for c_e in [0, 0.4]:
...
if __name__ == '__main__':
    ...
    # Export to R for ggplotting
    df.to_csv("experiment-a2-ci.csv")
